...
class AttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        temb: Optional[torch.FloatTensor]=None,
        control_condition_tuple: Optional[torch.FloatTensor]=None,
        upsample_size: Optional[int] = None,
        scale: float = 1.0,
    ) -> torch.FloatTensor:
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if control_condition_tuple is not None:
                control_condition = control_condition_tuple[-1]
                control_condition_tuple = control_condition_tuple[:-1]
                hidden_states = attn(hidden_states, **cross_attention_kwargs)+control_condition
            else:
                hidden_states = attn(hidden_states, **cross_attention_kwargs)

            # print("--------------hidden---------------------")
            # print(hidden_states.shape)
            # print(control_condition.shape)
            # print("----------------control-----------------")

            # print("------------------------------------")
            # for i in control_condition_tuple:
            #     print(i.shape)
            # print("------------------------------------")

            # print(f"attn_hidden_states.shape: {hidden_states.shape}")
        ...
        for resnet in self.resnets:
            ...
            if self.training and self.gradient_checkpointing:
                ...
                if control_condition_tuple is not None:
                    control_condition = control_condition_tuple[-1]
                    control_condition_tuple = control_condition_tuple[:-1]
                    hidden_states = resnet(hidden_states, temb, scale=scale)+control_condition
                else:
                    hidden_states = resnet(hidden_states, temb, scale=scale)
                ...
                # print(f"Up block hidden_states.shape:{hidden_states.shape}")
